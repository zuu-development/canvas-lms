...
docker composeで各コンテナを立ち上げます。

docker compose -f docker-compose.yml  -f ./docker-compose/rce-api.override.yml up -d

## webコンテナとjobコンテナ内でセットアップ

### Canvas LMS 環境の再構築と初期セットアップ

Canvas LMSの各ディレクトリやファイルを削除し、最新のソースコードをコピーして置き換えています。
その後、Ruby（Bundler）とNode.js（Yarn）の依存パッケージをインストールし、サンプル設定ファイルを実際の設定ファイルとして配置、さらにフロントエンド資産のビルドとデータベースの初期セットアップを行っています。
以下のコマンドを実行してください。
...
### Canvas LMS アセットコンパイルと Nginx 設定の適用

必要なディレクトリとファイルを作成して環境準備を行い、Canvas LMS の本番用アセットをコンパイルしています。
その後、最新の Nginx 設定ファイルを指定ディレクトリにコピーして反映させています。
...
jobコンテナ内で以下のコマンドを実行してジョブを実行してください。
